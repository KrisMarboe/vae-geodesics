...
def define_masks(D, num_transformations, strategy='checkerboard'):
    """
    Define masks for the coupling layers.

    Parameters:
    D: [int]
        The dimension of the data.
    num_transformations: [int]
        The number of transformations to define masks for.
    Returns:
    masks: [list of torch.Tensor]
        A list of masks for the coupling layers.
    """
    if strategy == 'checkerboard':
        ...
        masks = torch.randint(0, 2, (num_transformations, D))
        return masks
